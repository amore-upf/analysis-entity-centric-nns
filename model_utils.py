...
def get_indexed_predictions_with_targets(model, inputs, targets, sequence_bounds, cuda=False, collect_ensembles_preds=False):
    ...
    predictions, raw_scores = get_all_predictions_and_mean_loss(model, inputs, targets, sequence_bounds, batch_size=24, chunk_size=-1, loss_function=None, use_cuda=cuda, collect_ensembles_preds=collect_ensembles_preds)
...
# TODO Delete this in published version.
def extract_features(model, inputs, targets, sequence_bounds, cuda=False, collect_ensembles_preds=False):
    """
    Under progress (first try, then beautify -- if effort is worth it).
    """
    # For convenience during evaluation, this function will output token indices along with predictions and targets:
    token_indices = np.arange(len(targets), dtype=int)
    # Restrict to indices/targets for the given sequence bounds (subset of scenes/episodes to be processed):
    ...
    # TODO @Future: Take batch and chunk size from config file; currently it may run out of memory when testing (with cuda) on training set.
    predictions = _collect_features(model, inputs, targets, sequence_bounds,  collect_ensembles_preds=collect_ensembles_preds)
    ...
    return predictions, token_indices_subset_filtered

def _collect_features(model, X_data, y_data, sequence_bounds, use_cuda=False, volatile=False, collect_ensembles_preds=False):
    """
    Under progress (first try, then beautify -- if effort is worth it).
    """
                    ...
                    for i, output in enumerate(outputs):
                        ...
                        if len(outputs[i]) > 0:
                            ...
                            if bounds[i][0] not in all_predictions:
                                all_predictions[bounds[i][0]] = output.data
                            else:
                                all_predictions[bounds[i][0]] = torch.cat([all_predictions[bounds[i][0]], output.data])
                else:
                        ...
                        for i, output in enumerate(outputs_k):
                            ...
                            if len(outputs_k[i]) > 0:
                                ...
                                if bounds[i][0] not in all_predictions_k:
                                    all_predictions_k[bounds[i][0]] = output.data
                                else:
                                    all_predictions_k[bounds[i][0]] = torch.cat([all_predictions_k[bounds[i][0]], output.data])
    ...
    if not collect_ensembles_preds:
        ...
        for model_k in all_predictions_models:
            ...
            all_predictions_sorted[model_k] = all_predictions_sorted[model_k].numpy()
    ...
    return all_predictions_sorted

def extract_features_semeval(model, X_data, y_data, target_token_indices, sequence_bounds, cuda=False, collect_ensembles_preds=False):
    """
    Under progress (first try, then beautify -- if effort is worth it).
    """
    y_dash = np.concatenate([y_data[bounds[0]:bounds[1]] for bounds in sequence_bounds[sequence_bounds[:,0].argsort()]])
    y_sorted = y_dash[y_dash != NON_ENTITY_MENTION]
    y_inds = np.concatenate([target_token_indices[bounds[0]:bounds[1]] for bounds in sequence_bounds[sequence_bounds[:,0].argsort()]])
    y_inds_sorted = y_inds[y_dash != NON_ENTITY_MENTION]

    all_predictions_sorted = _collect_features_semeval(model, X_data, y_data, sequence_bounds, use_cuda=cuda, volatile=True, collect_ensembles_preds=collect_ensembles_preds)

    if cuda: 
        all_predictions_sorted = all_predictions_sorted.cpu()
    if not isinstance(all_predictions_sorted, dict):
        all_predictions_sorted = all_predictions_sorted.numpy()
    
    return all_predictions_sorted, y_sorted, y_inds_sorted


# TODO Delete this in published version.
def _collect_features_semeval(model, X_data, y_data, sequence_bounds, use_cuda=False, volatile=False, collect_ensembles_preds=False):
    """
    Under progress (first try, then beautify -- if effort is worth it).
    """
    batch_size = len(X_data)
    ...
    for X_batch, y_batch, bounds in yield_padded_batch_and_bounds(X_data, y_data, sequence_bounds, batch_size, chunk_size, shuffle_data=False, use_cuda=use_cuda):
        for X_batch_chunk, y_batch_chunk in zip(X_batch, y_batch):
            ...
            # Compute desired outputs mask and create input variable:
            mask = (y_batch_chunk != NON_ENTITY_MENTION).squeeze()
            padded_batch = autograd.Variable(X_batch_chunk, volatile=volatile)  # Volatile=True means requires_grad=False.
            ...
            if len(outputs) > 0:
                if not collect_ensembles_preds:
                    for i, output in enumerate(outputs):
                        # If any outputs were made for this sequence
                        if len(outputs[i]) > 0:
                            # Either insert them anew or concatenate them with previous outputs for the sequence:
                            if bounds[i][0] not in all_predictions:
                                all_predictions[bounds[i][0]] = output.data
                            else:
                                all_predictions[bounds[i][0]] = torch.cat([all_predictions[bounds[i][0]], output.data])
    ...
    if not collect_ensembles_preds:
        # Sort the outputs; compute the mean loss
        all_predictions_sorted = torch.cat([all_predictions[k] for k in sorted(all_predictions.keys())], dim=0)
    else:
        all_predictions_sorted = {}
        for model_k in all_predictions_models:
...
def _loss_from_outputs_batch(loss_function, outputs_per_chunk, targets_per_chunk, subsampling_per_chunk = None):
    """
    ...
    # Apply subsampling mask if necessary:
    if subsampling_per_chunk is not None:
        subsampling_packed, _ = tensor_utils.pack_batch_masked(subsampling_per_chunk, mask=mask)
        targets_packed[1 - subsampling_packed] = NON_ENTITY_MENTION
...
def train(model, X, y, train_sequence_bounds, validation_sequence_bounds, args, no_shuffle, logger):
    ...
    while epoch < args.epochs:
        ...
        if epoch != -1:
            ...
            for X_batch, y_batch, _ in yield_padded_batch_and_bounds(X, y, train_sequence_bounds, args.batch_size, args.chunk_size, shuffle_data, use_cuda=use_cuda):
                ...
                for X_batch_chunk, y_batch_chunk in zip(X_batch, y_batch):
                    ...
                    # compute mask where outputs matter for loss function
                    subsample_mask = None
                    if args.subsampling:
                        #                  ['i', 'me', 'my', 'myself', 'mine']
                        subsample_tokens = [2521, 3138, 3369, 3370, 3221]    # TODO: Hard-coded; won't work for different dataset!
                        subsample_weights = [.2, .3, .4, .4, .4]
                        subsample_mask = (y_batch_chunk != NON_ENTITY_MENTION)
                        for t, w in zip(subsample_tokens, subsample_weights):
                            if use_cuda:
                                bitmask = torch.cuda.FloatTensor(subsample_mask.size()).uniform_() < w
                            else:
                                bitmask = torch.FloatTensor(subsample_mask.size()).uniform_() < w
                            token_mask = (X_batch_chunk[:,:,0] == t).squeeze()
                            subsample_mask[token_mask] *= bitmask[token_mask]

                    # Apply model and calculate loss
                    outputs = model(padded_batch, desired_outputs_mask=mask, desired_outputs=autograd.Variable(y_batch_chunk))

                    loss = _loss_from_outputs_batch(loss_function, outputs, y_batch_chunk, subsampling_per_chunk=subsample_mask)
        ...
        # TODO Should these statistics also take into account subsampling?
        if (epoch + 1) % args.test_every == 0:
            ...
            if mean_val_loss < min_val_loss or performance[stop_criterion_data]["macro_f1_score"] > max_val_macro_f1:    # i.e., if the model is improving.
                num_epochs_no_improve = 0
                if performance[stop_criterion_data]["macro_f1_score"] > max_val_macro_f1:
                    best_model = model
                    max_val_macro_f1 = performance[stop_criterion_data]["macro_f1_score"]
            ...
            if num_epochs_no_improve >= args.stop_criterion:
                message = 'Stopped after epoch {0} because validation scores did not improve for {1} epochs.'.format(epoch,
                                                                                                                    num_epochs_no_improve)
                logger.say(message)
                logger.log('# ' + message)
                break

            if np.isnan(mean_train_loss):
                message = 'Stopped because training loss is NaN.'
                logger.say(message)
                logger.log('# ' + message)
                break
