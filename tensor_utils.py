...
def pad_sequences(list_of_tensors):
    ...
    """
    # TODO The following is an ugly last-minute fixes here to avoid error with empty tensor...
    seq_lengths = [(t.shape[0] if len(t.shape)>0 else 0) for t in list_of_tensors]
    max_seq_length_index = max(enumerate(seq_lengths), key=lambda x: x[1])[0]
    longest_tensor = list_of_tensors[max_seq_length_index]
    zeroes = torch.zeros_like(longest_tensor)
    padded_list = [(F.pad(t, (0, 0, 0, len(longest_tensor) - t.shape[0]), mode='constant', value=-1) if len(t.shape)>0 else zeroes.clone()) for t in list_of_tensors]
